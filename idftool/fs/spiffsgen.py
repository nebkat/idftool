"""SPIFFS image writer, vendored from ESP-IDF's ``components/spiffs/spiffsgen.py``.

SPDX-FileCopyrightText: 2019-2024 Espressif Systems (Shanghai) CO LTD
SPDX-License-Identifier: Apache-2.0

Espressif publishes no PyPI package for this generator, and SPIFFS's page allocation
is intricate enough that a re-implementation would drift from what the device expects,
so the classes are kept verbatim (minus the command-line front end) and driven from
:mod:`idftool.fs.spiffs`. Re-vendor by diffing against the ESP-IDF copy.
"""
import io
import math
import os
