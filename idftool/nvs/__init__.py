...
Detecting a pre-built NVS binary, fitting one to a partition, generating one from CSV via
``esp-idf-nvs-partition-gen``, and — through :mod:`idftool.nvs.parser`, which the generator
has no equivalent of — reading one back into key/value pairs. The commands live in
:mod:`idftool.commands.nvs`.
"""
import argparse
import csv
import io
...
def looks_like_nvs_binary(data: bytes) -> bool:
    ...
    Deliberately a cheap sniff rather than a full :func:`parse`, because it runs on
    files that are usually *not* NVS images and only has to tell the two apart.
    """
    if not data or len(data) % PAGE_SIZE != 0:
        return False

    blank_page = b'\xff' * PAGE_SIZE
    non_blank_pages = 0
...
#: Entry type → the ``encoding`` column that reproduces it through ``create-nvs``.
CSV_ENCODINGS = {'string': 'string', 'blob': 'hex2bin'}


def to_csv(entries: list[NvsEntry]) -> str:
    """Serialise parsed entries as an ``nvs_partition_gen`` CSV.

    The result round-trips: feeding it back to ``create-nvs`` rebuilds an image with the
    same contents. Blobs come out as ``hex2bin`` because that is the only binary encoding
    the CSV format can carry inline.
    """
    out = io.StringIO()
    ...
    for namespace in namespaces:
        ...
        for entry in sorted((e for e in entries if e.namespace == namespace),
                            key=lambda e: e.key):
            encoding = CSV_ENCODINGS.get(entry.type, entry.type)
            if entry.type == 'blob':
                value = entry.value.hex()
            else:
                value = str(entry.value)
            writer.writerow([entry.key, 'data', encoding, value])
